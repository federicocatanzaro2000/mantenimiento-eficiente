...
BEGIN
  ...
  IF TG_OP IN ('INSERT','UPDATE') THEN
    ...
    -- Regla general (para cualquier tipo de OIT):
    -- Si se marca explícitamente sin equipo, limpiar los datos del equipo.
...
-- Backfill de órdenes existentes: si tenían equipo cargado, quedan como "con equipo".
ALTER TABLE public.ordenes DISABLE TRIGGER USER;
UPDATE public.ordenes
SET project_has_equipment = CASE
  WHEN COALESCE(NULLIF(btrim(codigo_equipo), ''), NULLIF(btrim(nombre_equipo), '')) IS NOT NULL THEN true
  ELSE false
END
