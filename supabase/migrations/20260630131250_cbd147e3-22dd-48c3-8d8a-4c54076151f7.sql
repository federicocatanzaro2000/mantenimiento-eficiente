...
ALTER TABLE public.ordenes ADD COLUMN IF NOT EXISTS project_has_equipment boolean;

-- Backfill omitiendo triggers (incluye el de auditoría que requiere auth.uid())
SET LOCAL session_replication_role = 'replica';
UPDATE public.ordenes
SET project_has_equipment = (COALESCE(NULLIF(btrim(codigo_equipo), ''), NULLIF(btrim(nombre_equipo), '')) IS NOT NULL)
WHERE project_has_equipment IS NULL
  AND lower(btrim(coalesce(tipo_orden, ''))) LIKE 'proyecto%';
...
BEGIN
  ...
  IF TG_OP IN ('INSERT','UPDATE') THEN
    IF NEW.tecnicos_responsables IS NOT NULL AND array_length(NEW.tecnicos_responsables, 1) IS NOT NULL THEN
      NEW.tecnico_responsable := array_to_string(NEW.tecnicos_responsables, ', ');
    ELSIF NEW.tecnico_responsable IS NOT NULL AND length(btrim(NEW.tecnico_responsable)) > 0 THEN
      NEW.tecnicos_responsables := ARRAY[NEW.tecnico_responsable];
    ELSE
      NEW.tecnicos_responsables := '{}';
    END IF;

    IF lower(btrim(coalesce(NEW.tipo_orden, ''))) LIKE 'proyecto%' THEN
      IF NEW.project_has_equipment = false THEN
        NEW.codigo_equipo := '';
        NEW.nombre_equipo := '';
      END IF;
    ELSE
      NEW.project_has_equipment := NULL;
    END IF;
  END IF;
  ...
  IF TG_OP = 'UPDATE' THEN
    ...
    changed_1 := (NEW.nro_orden IS DISTINCT FROM OLD.nro_orden)
              ...
              OR (NEW.tecnicos_responsables IS DISTINCT FROM OLD.tecnicos_responsables)
    ...
    changed_3 := (NEW.codigo_documento IS DISTINCT FROM OLD.codigo_documento)
              ...
              OR (NEW.project_has_equipment IS DISTINCT FROM OLD.project_has_equipment);
