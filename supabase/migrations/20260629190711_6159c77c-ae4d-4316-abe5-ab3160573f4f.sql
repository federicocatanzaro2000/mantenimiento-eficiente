...
ALTER TABLE public.ordenes
  ADD COLUMN IF NOT EXISTS tecnicos_responsables text[] NOT NULL DEFAULT '{}';

-- Bypass auth trigger during backfill
SET LOCAL session_replication_role = 'replica';

UPDATE public.ordenes
  SET tecnicos_responsables = ARRAY[tecnico_responsable]
  WHERE (array_length(tecnicos_responsables,1) IS NULL)
    AND tecnico_responsable IS NOT NULL
    AND length(btrim(tecnico_responsable)) > 0;
...
CREATE INDEX IF NOT EXISTS idx_ordenes_tecnicos_responsables
  ON public.ordenes USING GIN (tecnicos_responsables);
